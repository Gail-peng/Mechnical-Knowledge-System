...
基于加工刀具知识图谱的知识系统。前端通过VUE2开发，后端通过Flask实现，数据库使用Neo4j。

数据来源于多个知名的刀具数据厂家，在整理了多家刀具数据后，设计了一套面向刀具数据的知识图谱本体。所有的数据都是遵循本体的规则存储到图数据库中。最后系统实现了图谱可视化、数据管理、产品查询、知识图谱结合大模型的RAG等功能。
